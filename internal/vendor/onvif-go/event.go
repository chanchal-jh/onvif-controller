...
func (c *Client) PullMessages(
	...
	for i := range resp.NotificationMessages {
		...
		msg := NotificationMessage{
			...
			SubscriptionID:  nm.SubscriptionReference.Address,
		}

		msg.Message.PropertyOperation = nm.Message.TTMessage.PropertyOperation

		if nm.Message.TTMessage.UtcTime != "" {
			if t, err := time.Parse(
				time.RFC3339Nano,
				nm.Message.TTMessage.UtcTime,
			); err == nil {
		...
		msg.Message.Source = make([]SimpleItem, len(nm.Message.TTMessage.Source.SimpleItems))
		for j, item := range nm.Message.TTMessage.Source.SimpleItems {
			msg.Message.Source[j] = SimpleItem(item)
		}

		// Convert key items.
		msg.Message.Key = make([]SimpleItem, len(nm.Message.TTMessage.Key.SimpleItems))
		for j, item := range nm.Message.TTMessage.Key.SimpleItems {
			msg.Message.Key[j] = SimpleItem(item)
		}

		// Convert data items.
		msg.Message.Data = make([]SimpleItem, len(nm.Message.TTMessage.Data.SimpleItems))
		for j, item := range nm.Message.TTMessage.Data.SimpleItems {
